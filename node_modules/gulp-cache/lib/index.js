...
class TaskProxy {
	...
	_storeCachedResult(key, result) {
		...
		return _asyncToGenerator(function* () {
			...
			return _this8._addCached(_this8.options.name, key, _JSON$stringify(files, null, 2));
		})();
	}
...
function plugin(task, inputOptions) {
	...
	if (!task) {
		throw new gulpUtil.PluginError('gulp-cache', 'Must pass a task to cache()');
	}
	...
	function each(file, enc, next) {
		...
		if (file.isStream()) {
			next(new gulpUtil.PluginError('gulp-cache', 'Cannot operate on stream sources'));
			return;
		}
		...
		signals.on('error', err => {
			next(new gulpUtil.PluginError('gulp-cache', err));
		});
...
plugin.clearAll = function clearAll() {
	return new _Promise((resolve, reject) => {
		fileCache.clear(null, err => {
			...
			if (err) {
				reject(new gulpUtil.PluginError('gulp-cache', `Problem clearing the cache: ${err.message}`));
				return;
			}
